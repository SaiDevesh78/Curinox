...
def preprocess_image(image: np.ndarray) -> np.ndarray:
    """Enhance contrast on glossy foil surfaces using CLAHE."""
    # Convert image to Lab color space to isolate brightness/contrast
...
def analyze_with_gemini_free(image_input: Any) -> Optional[Dict[str, Any]]:
    """Free Vision AI fallback using Gemini 1.5/2.5 Flash."""
    ...
    try:
        ...
        prompt = (
            "Examine this medicine package closely. Find the expiry date (EXP / EXPIRY).\n"
            "Return ONLY a JSON object with keys 'month' (integer 1-12) and 'year' (4-digit integer).\n"
            "If no expiry date is visible, return {\"month\": null, \"year\": null}."
        )
        ...
        if month and year:
            return {
                "month": int(month),
                "year": int(year),
                "raw": f"GEMINI_FREE_{month:02d}/{year}",
            }
    except Exception:
        pass  # Quietly fail over to error response if network issues occur
...
def detect_expiry_from_image(img_input: Any) -> Dict[str, Any]:
    ...
    # Pass both original and contrast-enhanced images through detection
    ...
    # ============================================================
    # FREE GEMINI VISION FALLBACK (Triggers ONLY if local OCR fails)
    # ============================================================
    gemini_result = analyze_with_gemini_free(img_input)
    if gemini_result:
        month = gemini_result["month"]
        year = gemini_result["year"]
        return {
            "expiry_detected": True,
            "expiry": f"{month:02d}/{year}",
            "month": month,
            "year": year,
            "source": gemini_result["raw"],
        }
    return {
        "expiry_detected": False,
        "expiry": None,
